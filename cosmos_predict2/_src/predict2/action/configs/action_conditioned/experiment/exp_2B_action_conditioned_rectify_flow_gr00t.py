...
AC_CHUNK_SINGLE_VIEW_2B_CMR_13FRAME_8NODES_OSS = LazyDict(
    dict(
        defaults=[
            "/experiment/2b_bridge_action_conditioned_oss",
            {"override /net": "cosmos_v1_2B_action_chunk_conditioned"},
            {"override /data_train": "cmr_versius_train"},
            {"override /data_val": "cmr_versius_val"},
            "_self_",
        ],
        job=dict(
            group="official_runs_vid2vid",
            name="cosmos_predict2p5_2B_action_conditioned_cmr_13frame_44D_8nodes_release_oss",
            project="cosmos_predict2_action_conditioned",
        ),
        model=dict(
            config=dict(
                state_t=1 + 12 // 4,
                net=dict(
                    action_dim=44,  # 30D actions + 14D state conditioning
                ),
            ),
        ),
        dataloader_train=dict(
            batch_size=16,  # Per-GPU batch size (512x288 resolution allows larger batches)
            # Effective batch: 8 nodes × 8 GPUs × 16 = 1024
            # sampler=dict(
            #     dataset=dict(num_frames=13, data_split="train"),
            # ),
            # dataset=dict(num_frames=13, data_split="train"),
        ),
        optimizer=dict(
            lr=1.6e-4,  # Linear scaling: 8 nodes (2x from 4 nodes) → lr = 8e-5 × 2 = 1.6e-4
            weight_decay=0.1,
        ),
    ),
    flags={"allow_objects": True},
...
# =============================================================================
# Open-H Multi-Embodiment: all Open-H datasets (CMR + dVRK + Stanford + Hamlyn + ...)
# Uses MixedLeRobotDataset with weighted sampling and action padding to 44D.
# =============================================================================
AC_CHUNK_SINGLE_VIEW_2B_OPEN_H_13FRAME_8NODES_OSS = LazyDict(
    dict(
        defaults=[
            "/experiment/2b_bridge_action_conditioned_oss",
            {"override /net": "cosmos_v1_2B_action_chunk_conditioned"},
            {"override /data_train": "open_h_multi_train"},
            {"override /data_val": "open_h_multi_val"},
            "_self_",
        ],
        job=dict(
            group="official_runs_vid2vid",
            name="cosmos_predict2p5_2B_action_conditioned_open_h-fixed_13frame_8nodes_release_oss",
            project="cosmos_predict2_action_conditioned",
        ),
        model=dict(
            config=dict(
                state_t=1 + 12 // 4,
                net=dict(
                    action_dim=44,  # Unified: all embodiments zero-padded to 44D (CMR max)
                ),
            ),
        ),
        dataloader_train=dict(
...
for _item, _item_wo_resume, _item_mock_wo_resume in [
    ...
    [
        AC_CHUNK_SINGLE_VIEW_2B_CMR_13FRAME_8NODES_OSS,
        *build_debug_runs(AC_CHUNK_SINGLE_VIEW_2B_CMR_13FRAME_8NODES_OSS),
    ],
    [
