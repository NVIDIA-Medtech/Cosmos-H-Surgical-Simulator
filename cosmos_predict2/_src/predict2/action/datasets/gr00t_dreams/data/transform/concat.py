...
class ConcatTransform(InvertibleModalityTransform):
    ...
    def apply(self, data: dict) -> dict:
        ...
        if "action" in grouped_keys:
            ...
            # Check the action dims (allowing for transformed representations)
            for key in self.action_concat_order:
                target_shapes = [self.action_dims[key]]
                if self.is_rotation_key(key) or "pose" in key:
                    target_shapes.append(3)  # Allow for axis angle
                    target_shapes.append(6)  # Allow for rot6d only
                    target_shapes.append(9)  # Allow for xyz + rot6d (from xyz + quat)
                    target_shapes.append(self.action_dims[key] + 2)  # quat->rot6d adds 2 dims
                assert data[key].shape[-1] in target_shapes, (
