...
#SBATCH --job-name=open-h-gtc-cp2.5
#SBATCH --nodes=8
...
#SBATCH --gres=gpu:8
#SBATCH --time=4:00:00
#SBATCH --output=logs/open-h-gtc-cp2.5_%A_%a.out
#SBATCH --error=logs/open-h-gtc-cp2.5_%A_%a.out
...
REPO_DEST="/lustre/fsw/portfolios/healthcareeng/users/lzbinden/imaginaire/output/repo/open-h-gtc-cp2.5/run_${TIMESTAMP}_${SLURM_JOB_NAME:-unknown}"
...
TIMESTAMP.....: ${TIMESTAMP}
REPO_ROOT.....: ${REPO_ROOT}
LOG_FILE......: ${REPO_ROOT}/logs/open-h-gtc-cp2.5_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.out
EOF
...
# NOTE: --container-mounts MUST be a single line with no spaces after commas.
# Pyxis/enroot treats whitespace+newlines as part of the mount path and fails.

CONTAINER_MOUNTS="$CODE_PATH:/workspace"
CONTAINER_MOUNTS="${CONTAINER_MOUNTS},/lustre/fsw/portfolios/healthcareeng/projects/healthcareeng_holoscan/datasets/Open-H/cmr-surgical-60hz-fixed:/CMR_Versius"
CONTAINER_MOUNTS="${CONTAINER_MOUNTS},/lustre/fsw/portfolios/healthcareeng/projects/healthcareeng_holoscan/datasets/Open-H:/lustre/fsw/portfolios/healthcareeng/projects/healthcareeng_holoscan/datasets/Open-H"
CONTAINER_MOUNTS="${CONTAINER_MOUNTS},/lustre/fsw/portfolios/healthcareeng/users/nigeln/cache/huggingface/lerobot/jhu:/lustre/fsw/portfolios/healthcareeng/users/nigeln/cache/huggingface/lerobot/jhu"
